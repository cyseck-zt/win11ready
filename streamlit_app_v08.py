...
APP_VERSION = "0.8"
SCCM_QUERY_PATH = "sccm_queries/windows11_readiness_export.sql"
...
REMEDIATIONS = {
    ...
    "Unsupported CPU": {
        "action": "Validate the processor against Microsoft's supported CPU list. If unsupported, plan for replacement.",
        "priority": "Critical",
        ...
        rows.append(
            {
                ...
                "CPU Support Detail": result.get("CpuSupportReason", "Not evaluated"),
...
def show_import_wizard():
    st.subheader("SCCM Import Wizard")
    st.write("Use this workflow to generate a CSV from SCCM with the columns Win11Ready expects.")
    st.markdown(
        """
        **Step 1:** Open SQL Server Management Studio and connect to the ConfigMgr site database.  
        **Step 2:** Run the query below.  
        **Step 3:** Export the results to CSV.  
        **Step 4:** Upload the CSV on the main Win11Ready page.
        """
    )
    try:
        with open(SCCM_QUERY_PATH, "r", encoding="utf-8") as query_file:
            query_text = query_file.read()
    except FileNotFoundError:
        query_text = "SCCM query file not found."
    st.code(query_text, language="sql")
    st.download_button(
        "Download SCCM SQL Query",
        data=query_text.encode("utf-8"),
        file_name="windows11_readiness_export.sql",
        mime="text/sql",
    )
...
def show_import_validation(df, missing_columns):
    ...
    if "CPU_Supported" not in df.columns:
        st.info("CPU_Supported is no longer required. Win11Ready now derives CPU support from the Processor column.")
...
def show_drilldown(details_df):
    ...
    device_info_columns = ["Manufacturer", "Model", "Processor", "CPU Support Detail", "RAM_GB", "Storage_GB", "TPM_Version", "SecureBoot", "Disk_Free_GB", "OSVersion"]
...
def show_refresh_planner(details_df, replacement_cost_per_device, fixable_count, replacement_count):
    ...
    if refresh_plan.empty:
        ...
        st.download_button("Download Refresh Plan CSV", data=refresh_plan.to_csv(index=False).encode("utf-8"), file_name="windows11_refresh_plan.csv", mime="text/csv")
    ...
    estimated_date, forecast_message = forecast_readiness_date(fixable_count, replacement_count, remediations_per_week, replacements_per_month)
...
def main():
    ...
    with st.sidebar:
        ...
        st.caption("SCCM-style column names are normalized automatically when possible. CPU support is derived from Processor.")
    ...
    tabs = st.tabs(["Overview", "Import Wizard", "Import Validation", "History", "Hardware", "Remediation", "Devices", "Drill-down", "Refresh Planner", "Reports"])
    with tabs[0]:
