...
def filter_details_dataframe(details_df, search, status_filter, category_filter):
    ...
    if search:
        filtered_df = filtered_df[
            filtered_df["ComputerName"].astype(str).str.contains(search, case=False, na=False)
        ]
...
def show_executive_summary(total, ready, not_ready, readiness_score, readiness_grade, failure_counts, replacement_count, fixable_count, estimated_replacement_cost):
    ...
    if failure_counts.empty:
        st.write(
            ...
            f"The readiness score is {readiness_score}% with a grade of {readiness_grade}."
        )
        return
    ...
    st.write(
        f"{total} devices were assessed. {ready} devices are ready for Windows 11 and {not_ready} require remediation. "
        f"The current readiness score is {readiness_score}% with a grade of {readiness_grade}. "
        f"{fixable_count} device(s) appear fixable through remediation, while {replacement_count} may require replacement. "
...
def show_reports(
    ...
    st.write("Generate executive-ready files from the current assessment.")
    ...
    with col3:
        st.download_button(
            ...
            data=remediation_df.to_csv(index=False).encode("utf-8"),
            file_name="windows11_readiness_remediation.csv",
            mime="text/csv",
        )
...
def main():
    ...
    st.title("Win11Ready - Windows 11 Readiness Analyzer")
    ...
    if missing_columns:
        st.warning(
            "Some required columns were not found after normalization: "
    ...
    overview_tab, hardware_tab, remediation_tab, devices_tab, drilldown_tab, reports_tab = st.tabs(
        ["Overview", "Hardware", "Remediation", "Devices", "Drill-down", "Reports"]
    )
    ...
    with remediation_tab:
        show_remediation(remediation_df)
    with devices_tab:
